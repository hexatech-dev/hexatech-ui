...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // Mobile (top-anchored): inset from the screen edges instead of
      // flush, so the toast reads as a compact floating banner rather than
...
const toastVariants = cva(
  // has-[.grid>:only-child]:py-4 — toaster.tsx (kept local per-repo) wraps
  // title/description in a `.grid.gap-1` div; when there's no description,
  // that div has exactly one child, so :only-child matches and the card
  // gets extra vertical padding instead of reading as a thin single-line
  // pill. Purely structural (no per-repo call site needs to change) — most
  // toast(...) calls across every product only ever pass a title.
  "group pointer-events-auto relative flex w-full items-center justify-between gap-3 overflow-hidden rounded-xl border p-3 pr-9 shadow-lg transition-all has-[.grid>:only-child]:py-4 data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    // only:text-base — when there's no sibling ToastDescription (the
    // common case across every product's toast(...) calls), a lone
    // text-sm line reads as an afterthought; bump it up one step so a
    // title-only toast still carries some visual weight on its own.
    className={cn("text-sm font-semibold only:text-base", className)}
    {...props}
  />
));
